...
theBins <- rbind(chr_1_bin, chr_2_bin)
theBins <- theBins[order(theBins$binID), ]
theNewBinID <- 1 : nrow(theBins)
theNewBin <- cbind(as.data.frame(theBins), newID = theNewBinID)
...
write.table(theNewBin, paste(theBedFile, ".", chr_1, "-", chr_2, ".newForHicPlotterInterMap.bed", sep = ''), quote = F, sep = "\t", row.names = F, col.names = F)

theNewMatrix <- theMatrixChr_1_Chr_2
oldID_1 <- as.character(theNewMatrix$binID_1)
oldID_2 <- as.character(theNewMatrix$binID_2)
newID_1 <- theNewBin[oldID_1, 'newID']
newID_2 <- theNewBin[oldID_2, 'newID']

theNewMatrix$binID_1 <- newID_1
theNewMatrix$binID_2 <- newID_2
write.table(theNewMatrix, paste(theMatrixFile, ".", chr_1, "-", chr_2, ".newForHicPlotterInterMap.matrix", sep = ''), quote = F, sep = "\t", row.names = F, col.names = F)
